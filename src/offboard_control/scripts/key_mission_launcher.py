...
class KeyMissionLauncher(Node):
    def __init__(self):
        ...
        self.get_logger().info(
            "Waiting for /gpio_keys/key1 or /gpio_keys/key2 to become true"
        )
    ...
    def _trigger(self, executable, topic):
        ...
        self.vision_timer = self.create_timer(
            self.vision_start_delay, self._start_vision
        )

    def _start_vision(self):
        self.vision_timer.cancel()
        self.get_logger().info(
            "Starting: ros2 launch track2vision tracked2vision.launch.py"
        )
        self._start_process(
            ["ros2", "launch", "track2vision", "tracked2vision.launch.py"]
        )
        self.get_logger().info(
            f"Starting {self.selected_task} in {self.offboard_start_delay:.1f} s"
        )
        self.offboard_timer = self.create_timer(
            self.offboard_start_delay, self._start_offboard
        )

    def _start_offboard(self):
        self.offboard_timer.cancel()
        self.get_logger().info(
            f"Starting: ros2 run offboard_control {self.selected_task}"
        )
        self._start_process(
            ["ros2", "run", "offboard_control", self.selected_task]
        )
    ...
    def _start_process(self, command):
        try:
            ...
            self.get_logger().error(
                f"Failed to start {' '.join(command)}: {error}"
            )
            rclpy.shutdown()
