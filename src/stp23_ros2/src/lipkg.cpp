...
bool LiPkg::Parse(const uint8_t *data, long len) {
  for (int i = 0; i < len; i++) {
    if (AnalysisOne(data[i])) { // parse a package is success
      ...
      latest_point_ = one_pkg_.back();
      frame_tmp_.insert(frame_tmp_.end(), one_pkg_.begin(), one_pkg_.end());
      if (frame_tmp_.size() > 20000) {
        frame_tmp_.erase(frame_tmp_.begin(), frame_tmp_.begin() + (frame_tmp_.size() - 12000));
      }
      is_pkg_ready_ = true;
    }
  }
