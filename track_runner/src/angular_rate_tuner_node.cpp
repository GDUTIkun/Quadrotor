...
private:
  ...
  void on_k_i_rate(const std_msgs::msg::Float64::SharedPtr msg)
  {
    k_i_rate_ = std::max(0.0, sanitize_finite(msg->data, k_i_rate_));
    reset_integral();
    RCLCPP_INFO(get_logger(), "k_i_rate set to %.3f", k_i_rate_);
    publish_status();
  }
